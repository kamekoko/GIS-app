'''This part is processing to GeoDataFrame or GeoSeries input'''
'''Processed data (output) is GeoDataFrame or GeoSeries too'''
...
#     else:
#         features = []
#         set_features(features, df_1)
#         set_features(features, df_2)
...
def processing(conn, processing_name, processing_val, gdf_data_1, gdf_data_2):
    ...
    if processing_name == 'set_crs':
        gdf = gdf_data_1
        gdf['geom'] = gdf['geom'].to_crs(processing_val)
    ...
    # 2.2 空間解析：オブジェクト選択：最近隣データ取得
        ...
        gdf = gis_a_so.select_nearest(gdf_data_1, processing_val)
    ...
    # 2.3 領域分析（バッファリング）
        ...
        gdf = gdf_data_1
        gdf['geom'] = gdf.geometry.to_crs(3857) #Googlemapなどで採用されているWebメルカトル法の平面直交座標
        gdf['geom'] = gdf.geometry.buffer(processing_val)
        gdf['geom'] = gdf.geometry.to_crs(4326)
